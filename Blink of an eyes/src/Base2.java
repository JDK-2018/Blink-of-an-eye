...
import javax.swing.*;
class TimeRunnables implements Runnable { // 시간을 체크 해주는 함수 
	public JLabel timerLable; // 새로운 라벨선언
...
public class Base2 extends JPanel{
	public JLabel [] label = new JLabel[5]; // 격자판에 보여줄 숫자 라벨
	...
	public void play() {
		...
		this.setLayout(null); // 레이아웃을 설정하여 내가 원하는 자리에 설정할 수 있겠끔 했음
		clearlabel = new JLabel("Clear!!");  // 게임을 클리어했을 때 보여주는 라벨을 "Clear!!"로 설정
		this.add(clearlabel); //컨테이너에 게임을 클리어했을 때 보여주는 라벨을 추가
		this.add(timerLabel); //컨테이너에 시간을 보여주는 라벨 추가
		this.add(restart); // 재시작 버튼 추가
		...
		this.setBackground(Color.BLACK); // 백그라운드를 검은색으로 설정
		timerLabel.setForeground(Color.WHITE); // 시간 라벨의 색상을 흰색으로 설정
		timerLabel.setBounds(270,0,50,50); // 시간 라벨의 위치와 크기 설정
		TimeRunnables runnable = new TimeRunnables(timerLabel); // 타이머 스레드로 사용할 객체 생성, 타이머 값을 출력할 레이블을 생성자에 전달
		...
		for(int i = 0; i<button.length;i++) { // 숫자를 클릭할 버튼을 원하는 갯수만큼 생성(배열의 크기만큼)
			...
			this.add(button[i]); // 컨테이너에 버튼을 추가해준다.
